...
def OrdenarDiccionario(diccionario_par, clave, descendente=True):
    ...
    swapDic ={}
    qVal=0 
    if type(diccionario_par) != dict or not (clave in diccionario_par.keys()):  
        return None                                
    if len(diccionario_par[list(diccionario_par.keys())[0]])>0: 
        qVal=len(diccionario_par[list(diccionario_par.keys())[0]])
        for i in range(0, qVal):
            contenedor={}
            for key in diccionario_par:
                contenedor[key]=diccionario_par[key][i]
            cambiaLis.append(contenedor)
        listaDics = sorted(cambiaLis, key=lambda d: d[clave], reverse=not (descendente))
        for key in diccionario_par:
            comodin = []
            for i in range(0, qVal):
                comodin.append(listaDics[i][key])
            swapDic[key] = comodin
    else:
